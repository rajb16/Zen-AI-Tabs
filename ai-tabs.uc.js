...
(() => {
  ...
  const applyUserStyles = () => {
    ...
    root.style.setProperty(
      ...
      showLabels ? "flex" : "none",
    );

    // 2. Line Gap
    root.style.setProperty("--zen-ai-separator-gap", `${gap}px`);

    // 3. Sizes (FIXED: Icon sizes reduced to match font sizes to prevent clipping)
    let iconSize = 13,
      fontSize = 12,
      btnPadding = "2px 6px";
  ...
  const domCache = {
    getSeparators() {
      return document.querySelectorAll(".pinned-tabs-container-separator");
    },
    getCommandSet() {
      return document.querySelector("commandset#zenCommandSet");
    },
  };
  ...
  const getFilteredTabs = (workspaceId, options = {}) => {
    ...
    return Array.from(gBrowser.tabs).filter((tab) => {
      ...
      const isInGroup = tab.hasAttribute("zen-group");
  ...
  const askAIForMultipleTopics = async (tabs) => {
    ...
    if (provider === 1) {
      ...
      if (currentWorkspaceId) {
        ...
        document
          .querySelectorAll(
            `.zen-custom-group-header[zen-workspace-id="${currentWorkspaceId}"]`,
          )
          .forEach((g) => {
            const l = g.getAttribute("group-name");
            if (l) groups.push(l);
          });
    ...
    if (currentWorkspaceId) {
      document
        ...
        .forEach((headerEl) => {
          const label = headerEl.getAttribute("group-name");
          if (label) {
            const groupTabs = Array.from(
              gBrowser.tabContainer.querySelectorAll(
                `tab[zen-workspace-id="${currentWorkspaceId}"][zen-group="${label}"]`,
              ),
            );
  ...
  const sortTabsByTopic = async () => {
    ...
    try {
      ...
      // Safety check: Ensure the Tab Groups mod is installed
      if (!window.ZenCustomGroups) {
        console.error(
          "[Zen AI Tabs] Required mod 'Zen Tab Groups' is missing.",
        );
        return;
      }
      ...
      document
        ...
        .forEach((g) => {
          existingGroupElementsMap.set(g.getAttribute("group-name"), g);
        });

      // Pause the evaluate chain so we don't trigger unnecessary re-sorts while moving
      window.ZenCustomGroups.isMovingMultiple = true;
      ...
      for (const topic in finalGroups) {
        ...
        if (headerEl && headerEl.isConnected) {
          ...
          if (headerEl.getAttribute("zen-collapsed") === "true") {
            headerEl.setAttribute("zen-collapsed", "false");
            const existingTabs = gBrowser.tabContainer.querySelectorAll(
      ...
      // Resume evaluate chain and clean up
      ...
      `
        <toolbarbutton id="sort-button" class="sort-button-with-icon" command="cmd_zenSortTabs" tooltiptext="Sort Tabs (AI)">
            <hbox class="toolbarbutton-box" align="center">
                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 28 28" class="broom-icon">
